...
if __name__ == '__main__':
    ...
    transform = Mytransforms.Compose([
                ...
                Mytransforms.RandomCrop(crop_size),
                #Mytransforms.RandomHorizontalFlip(),
            ])
    ...
    for i, data in enumerate(p):
        ...
        for i in range(1):
            ...
            heatmaps = cv2.resize(heatmaps, (crop_size, crop_size))
            for j in range(0, 1):
                print(heatmaps.shape)
                heatmap = heatmaps[:, :, j]
                heatmap = heatmap.reshape((crop_size, crop_size, 1))
                heatmap *= 255
            ...
            vecs = cv2.resize(vecs, (crop_size, crop_size))
            for j in range(0, 2, 2):
                vec = np.abs(vecs[:, :, j])
                vec += np.abs(vecs[:, :, j + 1])
                vec[vec > 1] = 1
                vec = vec.reshape((crop_size, crop_size, 1))
                # vec[vec > 0] = 1
                vec *= 255
                vec = cv2.applyColorMap(vec, cv2.COLORMAP_JET)
                vec = vec.reshape((crop_size, crop_size))
                vec /= 255
                print j
                plt.imshow(img)
